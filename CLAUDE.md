...
Vibeship is a Bulgarian-language vibe-coding platform built on Cloudflare, forked
from Cloudflare VibeSDK. The upstream agent, workspace and preview machinery is
unchanged; what Vibeship adds on top is:

- **Bulgarian UI.** All user-facing strings go through `src/i18n` (`useT()`), with
  `bg.ts` as the source of truth and `en.ts` as fallback. A missing key renders as
  the key itself so gaps show up in review. Never hardcode user-facing English.
- **Subscriptions and credits.** Plans, credit costs and packs live in
  `shared/types/billing.ts` and are shared by the Worker and the frontend.
  `worker/database/services/BillingService.ts` owns balances and the ledger;
  `worker/services/billing/` holds Stripe and the charge guards.
- **Design system.** The design lives in `design/Vibeship.dc.html`; its values are
  CSS variables named `--color-vs-*` in `src/index.css`. New screens use the
  primitives in `src/components/vibeship/ui.tsx`.

Anything that spends credits must charge through `chargeCredits` in
`worker/services/billing/creditGuard.ts` and refund on failure, so the ledger
stays a truthful record.
