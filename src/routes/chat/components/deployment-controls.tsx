...
export function DeploymentControls({
	...
	const handleToggleVisibility = async () => {
		...
		try {
			...
			toast.error('Видимостта не можа да бъде променена');
		} finally {
		...
		switch (state) {
			case DeploymentState.WAITING_PHASE1:
				return {
					...
					title: "Публикувай в Cloudflare",
					subtitle: deploymentTarget === 'user'
						? "Публикуването се отключва, щом проектът има файлове"
						: "Публикуването се отключва след първата фаза",
					buttonDisabled: true,
			...
			case DeploymentState.READY_TO_DEPLOY:
				return {
					...
					subtitle: `Качва самостоятелен Worker в ${destination}`,
			...
			case DeploymentState.DEPLOYING:
				return {
					...
					title: "Публикуване в Cloudflare",
					subtitle: `Приложението се качва в ${destination}…`,
					buttonDisabled: true,
					buttonVariant: "primary" as const,
			...
			case DeploymentState.REDEPLOYING:
				return {
					...
					titleColor: "text-blue-900 dark:text-blue-100",
					subtitleColor: "text-blue-600 dark:text-blue-300",
					title: "Ново публикуване в Cloudflare",
					subtitle: `Приложението се обновява в ${destination}…`,
					buttonDisabled: true,
					buttonVariant: "primary" as const,
					buttonClass: "bg-blue-500 hover:bg-blue-600 dark:bg-blue-600 dark:hover:bg-blue-700 text-white border-blue-500 dark:border-blue-600 scale-105 shadow-lg dark:shadow-blue-900/50"
				};
			...
			case DeploymentState.ERROR:
				return {
					...
					title: "❌ Публикуването се провали",
					subtitle: "Грешка при публикуването — опитай пак",
						...
						>
							{isCurrentlyDeploying ? (
								<>
									...
									{currentState === DeploymentState.REDEPLOYING ? 'Публикува се отново…' : 'Публикува се…'}
								</>
							) : (
								<>
									<Zap className="w-4 h-4 mr-2" />
									{deploymentTarget === 'user' ? 'Публикувай в моя акаунт' : 'Публикувай в Cloudflare'}
								</>
							)}
						</Button>
					</div>
			...
			{currentState === DeploymentState.DEPLOYED && (
				...
				>
					<div className="flex items-center gap-3 mb-3">
						...
						<div className="flex-1">
							...
							<div className="text-xs text-green-700 dark:text-green-300 mt-0.5">
								Приложението ти вече е живо в {destination}
							</div>
						</div>
					</div>
					...
					<div className="bg-kumo-base/60 dark:bg-bg-4/60 border border-green-200/40 dark:border-green-800/20 rounded-md p-3 mb-3">
						<div className="text-xs text-green-600 dark:text-green-400 font-medium mb-1">Адрес на живо:</div>
