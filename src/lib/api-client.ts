...
/** Отговорът на `/api/billing/plans`. */
export interface BillingPlansResponse {
	plans: Array<PlanDefinition & { purchasable: boolean }>;
	packs: Array<CreditPack & { purchasable: boolean }>;
...
class ApiClient {
	...
	// ===============================
	// Billing / Credits (Vibeship)
	// ===============================

	/** Планът, салдото, употребата и дневника на кредитите. */
	async getBillingSummary(): Promise<ApiResponse<BillingSummary>> {
		return this.request<BillingSummary>('/api/billing/summary');
	}

	/** Плановете и пакетите, налични в тази инсталация. */
	async getBillingPlans(): Promise<ApiResponse<BillingPlansResponse>> {
		return this.request<BillingPlansResponse>('/api/billing/plans');
	}

	/** Отваря Stripe Checkout за абонамент и връща адреса за пренасочване. */
	async createBillingCheckout(planId: string): Promise<ApiResponse<{ url: string }>> {
	...
	async createBillingTopup(packId: string): Promise<ApiResponse<{ url: string }>> {
		return this.request<{ url: string }>('/api/billing/topup', {
			method: 'POST',
			body: JSON.stringify({ packId }),
		});
	}

	/** Портал на Stripe за смяна на карта, отказ и фактури. */
	async createBillingPortal(): Promise<ApiResponse<{ url: string }>> {
