...
class SetupManager {
	...
	private async collectUserConfig(): Promise<void> {
		...
		if (useAIGateway) {
			...
			const gatewayName = wranglerConfig.vars?.CLOUDFLARE_AI_GATEWAY || 'vibeship-gateway';
	...
	private async ensureKVNamespace(binding: string): Promise<{ id: string; title: string }> {
		const namespaceName = `vibeship-${binding.toLowerCase()}-local`;
	...
	private async ensureAIGateway(resources: ResourceInfo): Promise<void> {
		const gatewayName = this.config.useAIGateway ? 'vibeship-gateway' : null;
