...
import { chargeCredits, refundCredits } from '../../services/billing/creditGuard';
    ...
    try {
        ...
        switch (parsedMessage.type) {
            ...
            case WebSocketMessageRequests.DEPLOY: {
                ...
                // Публикуването струва кредити — таксуваме преди да тръгне,
                // а при провал ги връщаме, за да не се плаща за нищо.
                const deployCharge = await chargeCredits(
                    agent.env,
                    agent.state.metadata.userId,
                    'deploy',
                    {
                        appId: agent.state.metadata.agentId ?? null,
                        description: 'Публикуване на Cloudflare',
                    },
                );
                if (!deployCharge.ok) {
                    sendToConnection(connection, WebSocketMessageResponses.ERROR, {
                        error: deployCharge.reason,
                ...
                agent.deployToCloudflare(target).then((deploymentResult) => {
                    if (!deploymentResult?.deploymentUrl) {
                        ...
                        void refundCredits(
                            agent.env,
                            agent.state.metadata.userId,
                            deployCharge.charged ?? 0,
                            {
                                appId: agent.state.metadata.agentId ?? null,
                                description: 'Върнати кредити за неуспешно публикуване',
                            },
                        );
                        return;
                    }
            ...
            case WebSocketMessageRequests.USER_SUGGESTION:
                ...
                // Всяко съобщение до агента струва кредит. Таксуваме преди
                // работата, защото инференцията вече е платена от нас в
                // момента, в който агентът тръгне.
                {
