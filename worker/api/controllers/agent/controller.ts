...
export class CodingAgentController extends BaseController {
    ...
    static async startCodeGeneration(request: Request, env: Env, _: ExecutionContext, context: RouteContext): Promise<Response> {
        try {
            ...
            // Лимитът за активни проекти идва от плана на потребителя.
            const appService = new AppService(env);
            const projectLimit = await checkProjectLimit(env, user.id, () =>
                appService.getUserAppsCount(user.id),
            );
            if (!projectLimit.ok) {
                return CodingAgentController.createErrorResponse(projectLimit.reason ?? 'Достигнат лимит за проекти', 403);
            }

            // Създаването на проект е най-скъпото действие — таксува се преди
            // да пуснем агента.
            const creationCharge = await chargeCredits(env, user.id, 'create', {
                description: 'Създаване на проект',
            });
