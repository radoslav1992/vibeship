...
export function createApp(env: Env): Hono<AppEnv> {
    ...
    app.use('*', async (c, next) => {
        ...
        // Stripe webhooks are server-to-server and carry no browser cookie, so
        // the double-submit token can never be present. They authenticate with
        // the `Stripe-Signature` HMAC instead, verified in the controller.
        if (new URL(c.req.url).pathname === '/api/billing/webhook') {
            return next();
        }
