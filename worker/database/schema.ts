...
// ========================================
// АБОНАМЕНТИ И КРЕДИТИ (VIBESHIP BILLING)
// ========================================

/**
 * Subscriptions table - един ред на потребител, описващ текущия му план.
 * Потребители без ред тук се третират като „free“.
 */
export const subscriptions = sqliteTable('subscriptions', {
    id: text('id').primaryKey(),
    userId: text('user_id').notNull().references(() => users.id, { onDelete: 'cascade' }),

    planId: text('plan_id', { enum: ['free', 'starter', 'pro', 'team'] }).notNull().default('free'),
    ...
    // Период на абонамента (за начисляване на кредити)
...
/**
 * Credit balances table - текущото салдо на потребителя.
 *
 * Разделено е на три кофи, защото се харчат в различен ред и изтичат различно:
 * месечната дажба (нулира се всеки период), пренесените кредити (валидни до
 * `rolloverExpiresAt`) и купените пакети (не изтичат).
 */
export const creditBalances = sqliteTable('credit_balances', {
    ...
    rolloverExpiresAt: integer('rollover_expires_at', { mode: 'timestamp' }),
    ...
    periodEndIdx: index('credit_balances_period_end_idx').on(table.periodEnd),
}));

/**
 * Credit ledger table - пълна история на движенията по кредитите.
 * Отрицателните суми са разход, положителните — начисляване.
 */
export const creditLedger = sqliteTable('credit_ledger', {
    ...
    kind: text('kind', { enum: ['grant', 'rollover', 'topup', 'spend', 'refund', 'expire'] }).notNull(),
    action: text('action', { enum: ['message', 'deploy', 'create', 'generation', 'index'] }),
    ...
    appIdx: index('credit_ledger_app_idx').on(table.appId),
    kindIdx: index('credit_ledger_kind_idx').on(table.kind),
}));

/**
 * Billing events table - идемпотентност за Stripe webhook-ите.
 * Stripe праща едно и също събитие повече от веднъж; записът тук пази
 * начисляването да не се случи два пъти.
 */
